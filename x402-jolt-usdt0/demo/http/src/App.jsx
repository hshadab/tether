...
function App() {
  ...
  const play = useCallback(async () => {
    ...
    for (let i = 0; i < SCENARIOS.length; i++) {
      ...
      await evProving;
      startProveTimer();
      await showCard({ type:'action', step:'proving', sc, label:'Generating zkML proof…', sub:'ONNX model running inside Jolt zkVM' }, 0);
...
function ProveTimer() {
  ...
  return (
    <div style={{ padding:'4px 10px', borderTop:'1px solid #334155', fontSize:'0.58rem', color:'#8b5cf6', fontWeight:700, display:'flex', justifyContent:'space-between' }}>
      ...
      <span style={{ color:'#64748b', fontWeight:400 }}>Real Jolt zkVM prover — no simulation</span>
    </div>
  );
}
    ...
    return (
      <div style={S.welcomeCard}>
        ...
        <p style={S.welcomeText}>
          When AI agents spend USDT0 autonomously via x402, users need more than promises that spending policy was followed — they need proof. Every payment passes through a cryptographically verified spending guardrail: a ZK proof, generated by a Jolt zkVM running an ML model, bound to the exact payment parameters. Change any field and the cryptographic guardrail rejects it.
        </p>
        <p style={S.welcomeSub}>
          Every proof is generated by a real Jolt zkVM prover. Every payment settles real USDT0 on Plasma via EIP-3009. Watch the balances change.
        </p>
        <p style={S.welcomeSub}>
        ...
        <div style={S.welcomePills}>
          ...
          <span style={S.wpill}><b style={{color:'#8b5cf6'}}>Jolt</b> zkVM proofs</span>
        </div>
        {/* model card */}
      ...
      <div style={S.overviewCard}>
        ...
        <div style={S.ovFlow}>
          {[
            ...
            { num:'2', title:'Agent loads zero knowledge machine learning (zkML) proof from ICME Labs', desc:'The zkML runs an ONNX ML model that evaluates agent spending policy and produces a proof. The proof is SHA-256 bound to the exact payment parameters.', color:'#8b5cf6' },
            ...
            { num:'5', title:'Cosigner verifies zkML proof', desc:'An independent Rust verifier confirms the ML model genuinely ran and approved the agent\u2019s transaction.', color:'#06b6d4' },
  ...
  if (c.type === 'action') {
    return (
      <div style={S.actionCard}>
        ...
        {c.step === 'proving' && (
          <div style={S.console}>
            <div style={S.conHdr}><span style={S.conDotR}/><span style={S.conDotY}/><span style={S.conDotG}/><span style={S.conTitle}>jolt zkvm prover</span></div>
            <div style={S.conBody}>
              ...
              <div style={{...S.conLn,animationDelay:'1.0s'}}><span style={S.conArrOut}>{'\u2192'}</span><span style={S.conDRes}>Running inference in Jolt zkVM...</span></div>
